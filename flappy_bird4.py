...
def main(genomes, config):
    ...
    while run:
        ...
        if len(birds) > 0:
            ...
            run = false
            break
        # als er geen vogels meer zijn sluit het spel af
...
def run(config_path):
    config = neat.config.Config(neat.DefaultGenome, neat.DefaultReproduction,
                                ...
                                config_path)
    # verteld hoe python bij het configuratie bestand moet komen van de AI
    
    p = neat.Population(config)
    # creëert de populatie
    p.add_reporter(neat.StdOutReporter(True))
    stats = neat.StatisticsReporter()
    p.add_reporter(stats)
    
    winner = p.run(main,50)
    #hoeveel generaties we maximaal willen, 50 dus
...
if __name__ == "__main__":
    ...
    # haalt de config uit de bestanden
    run(config_path)
    # start de config